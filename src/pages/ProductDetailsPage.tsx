...
export const ProductDetailPage = () => {
    ...
    const availableStock = product?.data?.remainingQuantityKg || product?.isActive || 0;
    const isOutOfStock = availableStock === 0;

    const handleAddToCart = () => {
        if (!product) return;

        const result = addToCart(product.data, quantity);
        toast({
            title: result.success ? 'Added to Cart!' : 'Error',
            description: result.success ? `${quantity} ${product.data?.name}(s) added to your cart` : result.message,
    ...
    const handleUserRating = (rating: number) => {
        ...
        setProduct((prev: any) => (prev ? { ...prev, rating } : prev));
    };
                ...
                <div className="space-y-6">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-2 py-1 rounded-full text-sm font-medium ${availableStock > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex items-center gap-3">
                                <Button variant="outline" size="sm" onClick={() => setQuantity(Math.max(0.1, quantity - 0.1))} disabled={quantity <= 0.1}>-</Button>
                                <span className="px-4 py-2 border rounded-md w-16 text-center">{quantity.toFixed(1)}</span>
                                <Button variant="outline" size="sm" onClick={() => setQuantity(Math.min(quantity + 0.1, availableStock))} disabled={quantity >= availableStock}>+</Button>
                                <span className="text-sm text-gray-600 ml-2">({availableStock} available)</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                            ...
                            <div className="flex items-center gap-2"><Shield className="h-4 w-4" /> Secure Payment</div>
                            <div className="flex items-center gap-2"><Package className="h-4 w-4" /> Easy Returns</div>
                        </div>
                        ...
                        <div className="mt-6">
                            ...
                            <div className="flex items-center space-x-1">
                                {[1, 2, 3, 4, 5].map((starValue) => (
                                    <Star
                                        key={starValue}
                                        className={`h-5 w-5 cursor-pointer transition-colors ${starValue <= (product.rating || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
